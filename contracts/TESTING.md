...
| Phase1 integrated loop | `test/Phase1IntegratedFlow.t.sol` | APPI apply -> daily UBI update -> UBI claim -> Covenant approval reward, Token A funded covenant -> crystallization -> template royalty split, delayed settle, dispute finalize settlement |
...
- a short documented "pre-merge" command set that developers actually run every time
- a second integrated scenario for issue acceptance and appeal covenant creation after resolution
