...
補足として、このシミュレーションは **制度の安定性と公平性を検証するための近似モデル** であり、`Token A` を現実の生活費へ厳密に校正したものではありません。物価校正の考え方自体は `docs/token_a_calibration_notes_ja.md` に分けて整理します。
...
Phase1 の後半または次段階では、次の軸もシミュレーションへ取り込む。

- 地域別 living-cost basket
- 最低食費・住居負担・通信費の proxy
- 村ごとの APPI baseline
- `Survival Buffer` を現実の最低生活コストへ近づけた再計算

この段階ではじめて、`100 Token A / 日` のような初期値を「制度確認用の数字」から「生活感覚に近い数字」へ寄せていく。
